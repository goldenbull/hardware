...
// map (-1, 1) to (0, 256, 0, 256, ...)
uint8_t acc2color(float v)
{
    if (v < -0.99)
        v = -0.99;
    if (v > 0.99)
    ...
    int c = (int)((v + 1) * 128 * 8);
    if (c % 512 <= 255)
        return (uint8_t)(c % 512);
    else
        return (uint8_t)(511 - c % 512);
}

void _RGB_Example2(void *arg)
{
    static uint8_t i = 0;
    while (1)
    {

        uint8_t r = acc2color(Accel.x);
        uint8_t g = acc2color(Accel.y);
        uint8_t b = acc2color(Accel.z);
        printf("%3d %3d %3d\n", r, g, b);
        Set_RGB(r, g, b);
